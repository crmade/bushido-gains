    ...
    <Card style={{ border: '1px solid ' + (complete ? C.green : C.line) }}>
      ...
      >
        <span className="flex items-center gap-2">
          <span style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 18, textTransform: 'uppercase' }}>{t('warmup.title')}</span>
          <span className="text-xs" style={{ color: C.dim }}>{t('warmup.duration')}</span>
        </span>
      ...
      {open && (
        <div className="mt-3 flex flex-col gap-3">
          ...
          <p className="text-xs mt-1" style={{ color: C.dim }}>{t('warmup.note')}</p>
        </div>
      )}
    </Card>
  );
